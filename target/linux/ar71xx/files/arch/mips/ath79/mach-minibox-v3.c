...
#define MINIBOX_V3_GPIO_LED_STATUS                14
#define MINIBOX_V3_GPIO_BTN_RST                   17
#define MINIBOX_V3_KEYS_POLL_INTERVAL             20	/* msecs */
#define MINIBOX_V3_KEYS_DEBOUNCE_INTERVAL         (3 * MINIBOX_V3_KEYS_POLL_INTERVAL)

#define MINIBOX_V3_MAC0_OFFSET                    0
#define MINIBOX_V3_MAC1_OFFSET                    6
...
static struct gpio_led minibox_v3_leds_gpio[] __initdata = {
	{
		...
		.gpio		= MINIBOX_V3_GPIO_LED_STATUS,
		.active_low	= 1,
	},
};
...
static struct gpio_keys_button minibox_v3_gpio_keys[] __initdata = {
	{
		...
		.gpio		= MINIBOX_V3_GPIO_BTN_RST,
		.active_low	= 1,
	},
};

static void __init minibox_v3_gpio_led_setup(void)
{
	ath79_register_leds_gpio(-1, ARRAY_SIZE(minibox_v3_leds_gpio),
			minibox_v3_leds_gpio);
	ath79_register_gpio_keys_polled(-1, MINIBOX_V3_KEYS_POLL_INTERVAL,
...
static void __init minibox_v3_setup(void)
{
	...
	minibox_v3_gpio_led_setup();

        ath79_register_usb();
     //   ath79_register_pci();

	ath79_register_wmac(art + MINIBOX_V3_WMAC_CALDATA_OFFSET, NULL);
	ath79_register_mdio(0, 0x0);
	ath79_register_mdio(1, 0x0);

	ath79_init_mac(ath79_eth0_data.mac_addr, art + MINIBOX_V3_MAC0_OFFSET, 0);
	ath79_init_mac(ath79_eth1_data.mac_addr, art + MINIBOX_V3_MAC1_OFFSET, 0);
...
MIPS_MACHINE(ATH79_MACH_MINIBOX_V3, "MINIBOX_V3", "Minibox V3 board", minibox_v3_setup);
